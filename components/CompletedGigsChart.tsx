...
interface CompletedGigsChartProps {
  days?: number
}

export function CompletedGigsChart({ days = 90 }: CompletedGigsChartProps) {
  const { data, isLoading, error } = useQuery({
    queryKey: ['completed-gigs', days],
    queryFn: async () => {
      const response = await fetch(`/api/admin/completed-gigs?days=${days}`)
      if (!response.ok) throw new Error('Failed to fetch completed gigs data')
    ...
    return (
      <div className="w-full">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-gray-500">Completed Gigs</h3>
  ...
  if (error || !data || data.length === 0) {
    const totalGigs = data ? data.reduce((sum, d) => sum + d.count, 0) : 0
    return (
      <div className="w-full">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-gray-500">Completed Gigs</h3>
          <span className="text-xs font-medium text-gray-700">
            {totalGigs > 0 ? `${totalGigs.toLocaleString()} gigs` : '0 gigs'}
          </span>
        </div>
        <div className="flex items-center justify-center h-48">
  ...
  const totalGigs = data.reduce((sum, d) => sum + d.count, 0)
      ...
      <ResponsiveContainer width="100%" height={180}>
        ...
        >
                ...
                return (
                  <div className="bg-white p-2 border border-gray-200 rounded shadow-sm">
                    ...
                    <p className="text-sm font-semibold text-gray-900">
                      {payload[0].value?.toLocaleString()} gigs
                    </p>
                  </div>
                )
              }
          ...
          <Line
            ...
            stroke="#73af17"
            strokeWidth={2}
            dot={{ fill: '#ffffff', stroke: '#73af17', strokeWidth: 2, r: 3 }}
            activeDot={{ r: 5, fill: '#ffffff', stroke: '#73af17', strokeWidth: 2 }}
          />
        </LineChart>
      </ResponsiveContainer>
