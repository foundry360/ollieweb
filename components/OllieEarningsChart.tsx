...
interface OllieEarningsChartProps {
  days?: number
}

export function OllieEarningsChart({ days = 90 }: OllieEarningsChartProps) {
  const { data, isLoading, error } = useQuery({
    queryKey: ['earnings-growth', days],
    queryFn: async () => {
      const response = await fetch(`/api/admin/earnings-growth?days=${days}`)
      if (!response.ok) throw new Error('Failed to fetch earnings growth data')
    ...
    return (
      <div className="w-full">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-gray-500">Ollie Earnings</h3>
  ...
  if (error || !data || data.length === 0) {
    const totalEarnings = data ? data.reduce((sum, d) => sum + d.amount, 0) : 0
    return (
      <div className="w-full">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-gray-500">Ollie Earnings</h3>
          <span className="text-xs font-medium text-gray-700">
            ${totalEarnings > 0 ? totalEarnings.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00'}
          </span>
        </div>
        <div className="flex items-center justify-center h-48">
