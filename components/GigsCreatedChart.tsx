...
interface GigsCreatedChartProps {
  days?: number
}

export function GigsCreatedChart({ days = 90 }: GigsCreatedChartProps) {
  const { data, isLoading, error } = useQuery({
    queryKey: ['gigs-growth', days],
    queryFn: async () => {
      const response = await fetch(`/api/admin/gigs-growth?days=${days}`)
      if (!response.ok) throw new Error('Failed to fetch gigs growth data')
    ...
    return (
      <div className="w-full">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-gray-500">Gigs Created</h3>
